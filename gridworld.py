...
#### This code has been adapted from https://github.com/lviano/RobustMCE_IRL/tree/master/reirl_code
class TabularEnv(gym.Env):
    ...
    def get_random_initial_index(self):
        return np.random.choice(range(self.n_states), p=self.p_in)
...
class GridWorldEnvironment(TabularEnv):
    def __init__(self, env_type=0, size=5, prop=0, w=None):
        ...
        self.reset()
        # Reward
        self.w = self.generate_w_terminal(env_type,w)
    ...
    def get_starting_index(self):
        ...
        if self.env_type == 1:
            return 24
        if self.env_type == 2:
            #choose randomly between all values between 0 and (size)**2-1
            return np.random.choice(range(self.size**2))
            #return 24
        if self.env_type == 9:
            return 24
        if self.env_type == 10:
            return 24
        if self.env_type == "preset":
            return np.random.choice(range(self.size**2))

    def generate_w_terminal(self, env_type, w_preset=None):
    ...
    def take_action(self, state, action):
        ...
        if self.is_in_grid(n_state):
            ...
            return state
    def is_terminal(self, state=None, state_id=None):
        if state_id is None:
            state_id = self.state_to_index(state)
        return state_id in self.terminal_indexes
    ...
    def step(self, a):
        ...
        return self.get_features(state_id = self.current_index), reward, self.done
    
    def state_step(self, state_index, a):
        p_next_index = self.P[a][state_index]
        next_index = np.random.choice(np.arange(self.n_states), p=p_next_index)

        if next_index in self.terminal_indexes:
            done = True
        else:
            done = False
        reward = self.state_r[next_index]

        return next_index, reward, done
